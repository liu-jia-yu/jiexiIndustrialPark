...
let CircleWaveMaterialType = 'CircleWaveMaterial'
let CircleWaveSource = `
...
class Effect {
  ...
  show(isShow){
    this.viewer.entities.show = isShow
  }
  getShow(){
    return this.viewer.entities.show
  }
  add(
    position,
    color,
...
class CircleWave extends Effect {
  ...
  add(position, color, maxRadius, duration, isedit = false, count = 3) {
    ...
    const _this = this
    const pro = new CircleWaveMaterialProperty({
      duration: duration,
      gradient: 0,
